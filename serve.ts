...
<head>
  ...
  <title>Resource Harmony — Login</title>
...
<body class="min-h-screen bg-gradient-to-br from-slate-50 via-indigo-50/30 to-white flex items-center justify-center p-4 relative overflow-hidden">
  <div class="absolute inset-0 overflow-hidden">
    <div class="absolute top-1/4 left-1/4 w-96 h-96 bg-indigo-200/30 rounded-full blur-3xl pulse-glow"></div>
    <div class="absolute bottom-1/4 right-1/4 w-80 h-80 bg-purple-200/20 rounded-full blur-3xl pulse-glow" style="animation-delay: 2s;"></div>
    <div class="absolute top-1/2 left-1/2 w-64 h-64 bg-blue-200/15 rounded-full blur-3xl pulse-glow" style="animation-delay: 4s;"></div>
  </div>
  <div class="relative z-10 w-full max-w-md">
    <div class="text-center mb-8 float">
      <div class="inline-flex items-center justify-center w-16 h-16 rounded-2xl bg-gradient-to-br from-indigo-500 to-indigo-700 mb-4 shadow-lg shadow-indigo-500/25">
        <svg class="w-8 h-8 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 19v-6a2 2 0 00-2-2H5a2 2 0 00-2 2v6a2 2 0 002 2h2a2 2 0 002-2zm0 0V9a2 2 0 012-2h2a2 2 0 012 2v10m-6 0a2 2 0 002 2h2a2 2 0 002-2m0 0V5a2 2 0 012-2h2a2 2 0 012 2v14a2 2 0 01-2 2h-2a2 2 0 01-2-2z"></path></svg>
      </div>
      <h1 class="text-2xl font-bold text-slate-900 tracking-tight">Resource Harmony</h1>
      <p class="text-slate-500 text-sm mt-1">Resource Management Platform</p>
    </div>
    <div class="bg-white/80 backdrop-blur-xl rounded-2xl p-8 shadow-xl shadow-indigo-500/5 border border-slate-200/60">
      ${errorHtml}
      <form method="POST" action="/__auth/verify" class="space-y-5">
        <div>
          <label class="block text-xs font-medium text-slate-600 uppercase tracking-wider mb-2">Email Address</label>
          <input type="email" name="email" required placeholder="you@company.com"
            class="w-full px-4 py-3 rounded-xl bg-slate-50 border border-slate-200 text-slate-900 placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-indigo-500/30 focus:border-indigo-500 transition-all text-sm">
        </div>
        <div>
          <label class="block text-xs font-medium text-slate-600 uppercase tracking-wider mb-2">Password</label>
          <input type="password" name="password" required placeholder="Enter your password"
            class="w-full px-4 py-3 rounded-xl bg-slate-50 border border-slate-200 text-slate-900 placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-indigo-500/30 focus:border-indigo-500 transition-all text-sm">
        </div>
        <button type="submit"
          class="w-full py-3 px-4 bg-indigo-600 hover:bg-indigo-700 text-white font-semibold rounded-xl transition-all duration-200 shadow-lg shadow-indigo-600/20 hover:shadow-indigo-600/30 text-sm">
          Sign In
        </button>
      </form>
      <p class="text-center text-slate-400 text-xs mt-6">Secured by Kapable Platform</p>
    </div>
  </div>
</body>
</html>`;
...
const server = Bun.serve({
  ...
  async fetch(request: Request): Promise<Response> {
    ...
    // SSE Proxy
    if (pathname === "/api/sse") {
      const tables = url.searchParams.get("tables") || "team_members,rh_projects,allocations";
...
console.log(`Resource Harmony listening on http://localhost:${server.port}`);
